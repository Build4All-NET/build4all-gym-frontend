...
import 'package:build4allgym/features/auth/data/models/AdminLoginResponse.dart';
...
class AuthApiService {
  ...
  Future<void> logoutRemote() async {
    ...
    final auth =
    access.toLowerCase().startsWith('bearer ') ? access : 'Bearer $access';
  ...
  Future<void> reactivateUser() async {
    ...
    final access = (await _tokenStore.getToken())?.trim() ?? '';
    if (access.isEmpty) {
      throw AuthException('Missing session token for reactivation',
          code: 'NO_TOKEN');
    }
  ...
  // ============================= TOKEN HELPERS ==========================
  Future<void> _storeAuthFromLogin(Map<String, dynamic> json,
